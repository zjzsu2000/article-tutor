...
State: copy-polish round committed and pushed (this commit) after two Codex rounds (round 1: two focused blockers; round 2: approved, no findings) — all nine previously deferred minor review findings are now cleared, leaving zero review debt. Weeks 4–5 (`3453f52`) and the question-bank expansion (`5511030`) are live on Vercel and the Cloudflare Pages mirror; this round's Cloudflare manual redeploy is pending approval. Next content round: Weekly Stories weeks 6–7.
...
- Weekly Stories track: weeks 1–5 are integrated; weeks 6–18 are "coming soon" and need original graded retellings + vocabulary + quiz before going live (see the content plan).
- Dictionary coverage: Weeks 1–5 vocabulary is in `lib/weeklyDictionary.ts`; Weeks 6–18 not yet imported. Lookup normalization is heuristic — irregular forms (e.g. `went`, `bought`, `swallowed`) and words absent from both dictionaries still show the friendly fallback. Function words (the/is/as) intentionally fall back.
